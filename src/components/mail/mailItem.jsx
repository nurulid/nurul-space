...
export const MailItem = ({user, email, avatar, time, title, desc, read, starred, attachment, label}) => {
  return (
          ...
          {(attachment.length > 0 || label.length > 0 || starred) && (
            <div className='flex justify-between items-center mt-5'>
              ...
              <div className='space-x-2'>
                {label?.map((item, i) => {
                  return (
                    <span key={i} className='inline-flex items-center gap-2 bg-gray-100 py-1 px-3 rounded-full text-gray-500 text-sm'>
